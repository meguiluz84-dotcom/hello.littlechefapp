...
interface Props {
  ...
  onComplete: (avatar: AvatarId, prefs: OnboardingPrefs, name: string) => void;
  showName?: boolean;
}
...
export default function Onboarding({ initialAvatar, onComplete, showName = true }: Props) {
  ...
  const finish = () => {
    ...
    onComplete(avatar, { age, restrictions: restr }, name.trim() || "Chef");
  };
        ...
        {step === 2 && (
          ...
          >
            ...
            <motion.button
              ...
              transition={{ repeat: Infinity, duration: 1.6 }}
              onClick={() => (showName ? setStep(3) : finish())}
              ...
              aria-label={showName ? "Siguiente" : "Empezar"}
            >{showName ? "➡️" : "✅"}</motion.button>
          </motion.div>
        )}
